...
figure,
plot(abs(rbs),'r'),title('Préambule B#');

% Déterminer le début du préambule B par cet algorithme
[max_value TDO_estime] = max(abs(moyenne));
seuil = 0.9*max(abs(rbs(TDO_estime-20:TDO_estime+20)))
for i = TDO_estime-20: TDO_estime+20
    if(abs(rbs(i))>seuil)
        debut_preambleB = i;
        break;
    end
...
preambule_c_apres_64=preambule_c(97:Taille_preambleB); 
...
figure,
stem(abs(spc_recu1),'b*-')
hold on
stem(abs(spc_recu2),'r-')
...
figure,
plot(real(Matsymb),imag(Matsymb),'+');
title(['Constellation de Réception apres ajouter la puissance M = ',num2str(M)]);
...
A = imread('image.jpg');
figure
imshow(A),title('image du signal1.3.mat ');
